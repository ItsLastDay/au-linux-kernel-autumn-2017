...
{
    size_t possible_read_size = read_size;
    if (read_size == 0) {
        return 0;
    }
    ...
    if (possible_read_size > vsd_dev->buf_size - *fpos) {
        possible_read_size = vsd_dev->buf_size - *fpos;
    } 

    if (copy_to_user(read_user_buf, 
                      vsd_dev->vbuf,
                      possible_read_size)) {
        return -EFAULT;
    }

    *fpos += possible_read_size;

    return possible_read_size;
}
...
static long vsd_dev_ioctl(struct file *file, unsigned int cmd,
        unsigned long arg)
{
    switch (cmd) {
