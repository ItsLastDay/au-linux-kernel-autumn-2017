...
static ssize_t flush_registers_and_set_result(ssize_t res) {
    dev.hwregs->result = res;
    wmb();
...
static ssize_t vsd_dev_read(char *dst, size_t dst_size, size_t offset) {
    if (offset >= dev.hwregs->dev_size) {
        return flush_registers_and_set_result(0);
    }
    if (offset + dst_size >= dev.hwregs->dev_size) {
        dst_size = dev.hwregs->dev_size - offset;
    }
    memcpy(dst, dev.vbuf + offset, dst_size); 
    return flush_registers_and_set_result(dst_size);
}

static ssize_t vsd_dev_write(char *src, size_t src_size, size_t offset) {
    if (offset >= dev.hwregs->dev_size) {
        return flush_registers_and_set_result(-EINVAL);
    }
    if (offset + src_size >= dev.hwregs->dev_size) {
        src_size = dev.hwregs->dev_size - offset;
    }
    memcpy(dev.vbuf + offset, src, src_size);
    return flush_registers_and_set_result(src_size);
}
...
static void vsd_dev_set_size(size_t size)
{
    ...
    flush_registers_and_set_result(ret);
}
...
static int vsd_dev_cmd_poll_kthread_func(void *data)
{
    ...
    while(!kthread_should_stop()) {
        ...
        if (dev.hwregs->tasklet_vaddr != 0) {
            void* tasklet = dev.hwregs->tasklet_vaddr;
